...
export function buildAssignmentPdf(assignment: PublicAssignment, fontBase64: string) {
  ...
  const value = (content: unknown, x: number, top: number, maxWidth: number) => {
    color(colors.ink);
    pdf.setFontSize(10);
  ...
  value(`${assignment.startTime} - ${assignment.endTime}`, margin + 64, y + 16, 45);
  ...
  const destinationHeight = value(assignment.destination, margin + 94, y + 7, 84);
  ...
  value(`${assignment.vehicle.licensePlate} - ${assignment.vehicle.brand} ${assignment.vehicle.model}`, margin + 6, y + 17, 78);
  ...
  if (assignment.notes) {
    fill([249, 250, 252]);
    stroke(colors.line);
    const allNoteLines = pdf.splitTextToSize(assignment.notes, contentWidth - 12);
    const noteLines = allNoteLines.slice(0, 3);
    if (allNoteLines.length > 3) noteLines[2] = `${noteLines[2].replace(/\s+$/, '')}...`;
  ...
  color(colors.muted);
  pdf.setFontSize(7.5);
  pdf.text('Generated by TOKIN Transport. Please present this document when requested.', margin, 288);
  pdf.text(`Assigned: ${text(assignment.assignedAt).slice(0, 19).replace('T', ' ')}`, width - margin, 288, { align: 'right' });
