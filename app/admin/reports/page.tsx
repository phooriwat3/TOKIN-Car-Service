...
function LegacyReports() {
  ...
  const exportExcel = async () => {
    ...
    data.bookings.forEach((booking) =>
      bookings.addRow({
        ...
        fuel: booking.tripLog?.fuelCost || 0,
        toll: booking.tripLog?.tollFee || 0,
        parking: booking.tripLog?.parkingFee || 0,
    ...
    dispatchGroups.forEach((group) =>
      group.passengers.forEach((passenger) =>
        dispatch.addRow({
          ...
          date: group.usingDate,
          time: group.startTime + " - " + group.endTime,
          plate: group.licensePlate,
          vehicle: group.brand + " " + group.vehicleType,
          driver: group.driverName,
          phone: group.driverPhone,
          employeeId: passenger.employeeId,
          employeeName: passenger.employeeName,
          busStop: passenger.busStop,
        }),
      ),
    );
  ...
  const exportPdf = async () => {
    ...
    dispatchGroups.forEach((group) => {
      ...
      pdf.setFontSize(11);
      pdf.text(
        group.licensePlate + " | " + group.brand + " " + group.vehicleType,
        14,
        y,
      );
      y += 6;
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-3 print:hidden">
        ...
        <Metric
          label="OT vehicles arranged"
          value={String(dispatchGroups.length)}
        />
        <Metric
          label="OT passengers arranged"
          value={String(
            dispatchGroups.reduce(
              (sum, group) => sum + group.passengers.length,
              0,
            ),
          )}
        />
      ...
      <section className="mt-6">
        <div className="mb-4 flex items-center gap-2">
          ...
          <div>
            <h2 className="font-bold text-ink">OT vehicle dispatch list</h2>
            <p className="text-sm text-gray-500">
              One list per vehicle for the transport service provider.
            </p>
          </div>
        </div>
              ...
              >
                <div className="flex flex-wrap justify-between gap-3 bg-brand px-5 py-4 text-white">
                  <div>
                    ...
                    <h3 className="text-xl font-bold">{group.licensePlate}</h3>
                    <p className="text-sm text-blue-100">
                      {group.brand} {group.vehicleType}
                    </p>
                  </div>
                  <div className="text-right text-sm">
                    <p>
                      {group.usingDate} ? {group.startTime} - {group.endTime}
                    </p>
                    <p className="mt-1 text-blue-100">{group.requestNo}</p>
                  </div>
                </div>
                <div className="grid gap-3 border-b border-line bg-blue-50 px-5 py-3 text-sm sm:grid-cols-2">
                  ...
                  <p>
                    ...
                    <strong>{group.driverPhone || "-"}</strong>
                  </p>
                </div>
                  ...
                  <table className="w-full min-w-[620px] text-left text-sm">
                    <thead className="bg-gray-50 text-xs uppercase text-gray-500">
                      <tr>
                        ...
                        <th className="px-5 py-3">Drop-off point</th>
                      </tr>
                    </thead>
                    <tbody>
                      {group.passengers.map((passenger, index) => (
                        <tr
                          key={passenger.employeeId}
                          className="border-t border-line"
                        >
                          <td className="px-5 py-3">{index + 1}</td>
                          <td className="px-5 py-3">{passenger.employeeId}</td>
                          <td className="px-5 py-3 font-semibold">
                            {passenger.employeeName}
                          </td>
                          <td className="px-5 py-3">{passenger.busStop}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                </div>
                <div className="px-5 py-3 text-right text-xs font-semibold text-gray-500">
                  Total passengers: {group.passengers.length}
                </div>
              </Card>
            ))
          ) : (
            <Card className="p-8 text-center text-sm text-gray-500">
              No confirmed OT vehicle assignments yet.
            </Card>
          )}
        </div>
      </section>
