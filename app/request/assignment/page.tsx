    ...
    return (
      <Page>
        <Card className="p-10 text-center">
          ...
          <h1 className="mt-3 text-xl font-bold">Assignment unavailable</h1>
    ...
    <Page>
      ...
      <Card className="overflow-hidden">
        <div className="grid gap-px bg-line sm:grid-cols-3">
          ...
          <Summary
            ...
            label="Passengers"
            value={String(assignment.numPassengers)}
          />
        </div>
      </Card>
      ...
      <Card className="mt-5 p-6">
        ...
        <div className="mt-5 border-t border-line pt-5">
          <Info label="Purpose" value={assignment.purpose} />
        </div>
      </Card>
      ...
      {assignment.transportUnits?.length ? (
        <div className="mt-5 space-y-4">
          <h2 className="font-bold">
            Vehicles and drivers ({assignment.transportUnits.length})
          </h2>
            ...
            <div key={index} className="grid gap-5 md:grid-cols-2">
              <Card className="border-l-4 border-l-brand p-6">
                <div className="flex items-center gap-2 text-brand">
                  <Car size={20} />
                  <h3 className="font-bold">Vehicle {index + 1}</h3>
                </div>
                <p className="mt-4 text-xl font-bold">{unit.licensePlate}</p>
                <p className="mt-1 text-sm text-gray-600">
                  {unit.brand} {unit.vehicleType}
                </p>
              </Card>
              <Card className="border-l-4 border-l-accent p-6">
                <div className="flex items-center gap-2 text-brand">
                  <UserRound size={20} />
                  <h3 className="font-bold">Driver {index + 1}</h3>
                </div>
              ...
              <Card className="p-6 md:col-span-2">
                <div className="flex items-center gap-2 text-brand">
                  ...
                  <h3 className="font-bold">
                    Passengers ({unit.employeeIds?.length || 0})
                  </h3>
                </div>
      ...
      {assignment.notes && (
        <Card className="mt-5 bg-amber-50 p-6">
          <h2 className="font-bold text-amber-900">Assignment notes</h2>
      ...
      <Card className="mt-5 p-6">
        <div className="grid gap-4 sm:grid-cols-2">
          <Info label="Requester" value={assignment.requester.name} />
          <Info label="Department" value={assignment.requester.department} />
        </div>
        <div className="mt-6 flex flex-wrap gap-3">
          <a
            href={pdfUrl}
            className="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-brand px-4 text-sm font-semibold text-white hover:bg-[#194786]"
          >
            <Download size={17} />
            Download PDF
          </a>
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-line bg-white px-4 text-sm font-semibold text-ink hover:bg-gray-50"
          >
            <MapPin size={17} />
            Get directions
          </a>
        ...
        {result.expiresAt && (
          <p className="mt-4 text-xs text-gray-500">
            This secure document link expires on{" "}
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-7 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Secure assignment document</p>
          </div>
        </div>
        {children}
      </div>
