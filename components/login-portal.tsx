...
export function LoginPortal({ admin = false }: { admin?: boolean }) {
  const { configured, authenticated, loading, error, role, setRole, signIn } =
    useApp();
  ...
  return (
    <main
      ...
      style={{
        background:
          "linear-gradient(135deg, #001F3F 0%, #00498E 50%, #0074D9 100%)",
      }}
    >
      <div
        className="pointer-events-none absolute -top-32 -right-32 h-[500px] w-[500px] opacity-20"
        style={{
          background: "radial-gradient(circle, #0074D9, transparent 70%)",
        }}
      />
      <div
      ...
      <div className="relative z-10 w-full max-w-md animate-scale-in">
        <div className="mb-8 flex flex-col items-center gap-3 text-white">
          <div className="flex h-16 w-36 items-center justify-center rounded-lg bg-white p-3 shadow-md">
            <img src="/tokin-logo.png" alt="TOKIN Logo" className="h-full w-full object-contain" />
          </div>
        ...
        >
          ...
          <div className="mt-6 border-t border-line pt-5 text-center text-sm">
            {admin ? (
              ...
              <Link
                className="font-medium text-gray-500 hover:text-brand transition"
                href="/admin/login"
              >
