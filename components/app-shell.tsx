...
export function AppShell({ children }: { children: React.ReactNode }) {
    ...
    const nav = <><div className="flex h-16 items-center gap-3 border-b border-white/10 px-5"><div className="grid h-9 w-9 place-items-center rounded-md bg-white text-brand font-bold">TT</div><div><p className="font-bold text-white">TOKIN Transport</p><p className="text-xs text-blue-200">{role === 'admin' ? 'Administration Portal' : role === 'requester' ? 'Request Portal' : 'Operations Portal'}</p></div></div><nav className="flex-1 space-y-1 p-3">{links[role].map(x => {
    ...
    </select><Button variant="ghost" title="Logout" onClick={async () => { await signOut(); router.push(loginPath); router.refresh() }}><LogOut size={17} /></Button></div></header><main className="mx-auto max-w-[1440px] p-4 md:p-7">{children}</main></div></div>
}
