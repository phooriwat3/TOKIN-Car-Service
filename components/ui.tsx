...
export type StatTone = "blue" | "amber" | "green" | "violet" | "red";
...
export function Stat({
  ...
  const card = (
    <div
      className={cn(
        "rounded-xl border border-slate-200 bg-white p-5 shadow-2xs transition-all duration-150",
        href && "hover:border-slate-300 hover:shadow-sm cursor-pointer",
        className,
      )}
    >
      <p className="text-xs font-semibold text-slate-500">{label}</p>
      <p className="mt-2.5 text-3xl font-bold font-sans tracking-tight text-slate-900 tabular-nums sm:text-[32px]">
        {value}
      </p>
    </div>
  );
  ...
  if (href) {
    return (
      <Link
        href={href}
        className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/20 rounded-xl"
      >
        {card}
      </Link>
    );
