...
begin
  ...
  if selected_approver.id is null then raise exception 'Approver is unavailable.'; end if;
  ...
  update public.bookings set
    status = 'pending_approval', request_type = p_request_type,
    approver_id = selected_approver.id, approver_name = selected_approver.full_name,
    approver_email = selected_approver.email,
    category = case when p_request_type = 'overtime' then 'overtime_transport' else 'business_trip' end,
    using_date = p_using_date, start_time = p_start_time, end_time = p_end_time,
    pickup_location = p_pickup_location, destination = p_destination, purpose = p_purpose,
    num_passengers = passenger_count, meeting_point = p_meeting_point,
    with_staff = p_with_staff, after_hours = p_request_type = 'overtime',
    overtime_transport = p_request_type = 'overtime', reject_reason = null, updated_at = now()
  ...
  if p_request_type = 'outside_company' then
    for passenger in select value from jsonb_array_elements(coalesce(p_passengers, '[]'::jsonb)) loop
      ...
      values (p_booking_id, trim(both '"' from passenger::text),
    ...
    for employee in select value from jsonb_array_elements(coalesce(p_overtime_employees, '[]'::jsonb)) loop
      insert into public.overtime_employees (
        ...
        p_booking_id, employee ->> 'employeeId', employee ->> 'employeeName',
        employee ->> 'workDescription', (employee ->> 'workStart')::time,
        (employee ->> 'workEnd')::time, (employee ->> 'totalWeeklyHours')::numeric,
