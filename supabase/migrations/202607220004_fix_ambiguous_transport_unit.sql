...
begin
  ...
  for transport_unit in
    select transport_element.value
    from jsonb_array_elements(p_transport_units) as transport_element(value)
  loop
    if nullif(btrim(transport_unit->>'unitId'), '') is null
      or nullif(btrim(transport_unit->>'licensePlate'), '') is null
      or nullif(btrim(transport_unit->>'brand'), '') is null
      or nullif(btrim(transport_unit->>'vehicleType'), '') is null
      or nullif(btrim(transport_unit->>'driverName'), '') is null
      or jsonb_typeof(transport_unit->'employeeIds') <> 'array'
      or jsonb_array_length(transport_unit->'employeeIds') = 0 then
  ...
  select coalesce(array_agg(assigned_employee.employee_id order by assigned_employee.employee_id), array[]::text[])
  into assigned_employee_ids
  from (
    select jsonb_array_elements_text(transport_element.value->'employeeIds') as employee_id
    from jsonb_array_elements(p_transport_units) as transport_element(value)
